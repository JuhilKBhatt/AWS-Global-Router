...
import { Region } from '../api/vpnClient';
...
const STORAGE_KEY = 'aws_global_router_settings';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ regions, onSettingsSaved }) => {
  ...
  const handleFinish = (values: AppSettings) => {
    try {
  ...
  const handleReset = () => {
    ...
    message.info('Reset to factory defaults.');
    if (onSettingsSaved) {
      onSettingsSaved(DEFAULT_SETTINGS);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 880, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <Text type="secondary">
          Configure idle session auto-teardown, spending budgets, and default deployment regions.
        </Text>
      </div>
